...
if missing_columns:
    ...
    with col_2: 
        with st.expander("Nuova riparazione", expanded=True):
            ...
            while attempts < max_attempts and not success:
                try:
                    with st.form("nuova_riparazione_form"):
                        st.session_state['input_data']['first_name'] = st.text_input("Nome", st.session_state['input_data'].get('first_name', ''))
                        st.session_state['input_data']['last_name'] = st.text_input("Cognome", st.session_state['input_data'].get('last_name', ''))
                        st.session_state['input_data']['phone_number'] = st.text_input("Numero di telefono", st.session_state['input_data'].get('phone_number', ''))
                        st.session_state['input_data']['device'] = st.selectbox("Tipo dispositivo", options=["TV", "PC", "Smartphone", "Tablet"], index=["TV", "PC", "Smartphone", "Tablet"].index(st.session_state['input_data'].get('device', 'TV')))
                        st.session_state['input_data']['brand'] = st.text_input("Marca", st.session_state['input_data'].get('brand', ''))
                        st.session_state['input_data']['model'] = st.text_input("Modello", st.session_state['input_data'].get('model', ''))
                        st.session_state['input_data']['price'] = st.number_input("Prezzo", value=st.session_state['input_data'].get('price', 1.00), format="%.2f", min_value=1.00)
                        st.session_state['input_data']['acconto'] = st.text_input("Acconto", st.session_state['input_data'].get('acconto', ''))
                        st.session_state['input_data']['operator'] = st.text_input("Operatore", st.session_state['input_data'].get('operator', ''))
                        st.session_state['input_data']['unlock_code'] = st.text_input("Codice sblocco", st.session_state['input_data'].get('unlock_code', ''))
                        st.session_state['input_data']['color'] = st.text_input("Colore", st.session_state['input_data'].get('color', ''))
                        st.session_state['input_data']['action'] = st.text_input("Azione", st.session_state['input_data'].get('action', ''))
                        st.session_state['input_data']['url'] = st.text_input("Link", st.session_state['input_data'].get('url', ''))
                        st.session_state['input_data']['muletto'] = str(st.checkbox("Muletto", st.session_state['input_data'].get('muletto', False)))
                        st.session_state['input_data']['left_accessory'] = str(st.checkbox("Accessori lasciati", st.session_state['input_data'].get('left_accessory', False)))

                        if st.session_state['input_data']['left_accessory']:
                            st.session_state['input_data']['descrizione_acessory_lasciati'] = st.text_input("Info accessori lasciati", st.session_state['input_data'].get('descrizione_acessory_lasciati'))
                    ...
                    success = True  # Esce dal ciclo se il form viene creato correttamente
                except KeyError:
                    attempts += 1
                    time.sleep(2)

            if not success:
                st.error("Impossibile creare il form. Si prega di riprovare più tardi.")
